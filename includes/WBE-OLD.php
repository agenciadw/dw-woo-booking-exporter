...
if (!class_exists('WBE')) {
    class WBE {
        ...
        public function enqueue_admin_js_date_range_picker($hook) {
            wp_enqueue_style('wbe-jquery-ui', plugins_url('../css/jquery-ui.min.css', __FILE__));
            wp_enqueue_style('wbe-jquery-ui-theme', plugins_url('../css/jquery-ui.theme.min.css', __FILE__));
            wp_enqueue_style('wbe-chosen', plugins_url('../css/chosen.min.css', __FILE__));
            wp_enqueue_style('wbe-plugin', plugins_url('../css/plugin.css', __FILE__));
            wp_enqueue_script('wbe-jquery-ui', plugins_url('../js/jquery-ui.min.js', __FILE__), array('jquery'));
            wp_enqueue_script('wbe-chosen', plugins_url('../js/chosen.jquery.min.js', __FILE__), array('jquery'));
            wp_enqueue_script('plugin-daterangepicker', plugins_url('../js/plugin.js', __FILE__), array('jquery'));
        ...
        public function export_booking_to_cv_pdf() {
            ...
            if (count($dataArray) == 0) { //if no rows Redirect again to this page
                ...
                // Preparação do cabeçalho do CSV e linhas
                ...
                foreach ($dataArray as $row) {
                    ...
                    // Remove os valores antigos e insere os novos
        ...
        public function wbe_inactive_plugin_notice(){ ?>
            <div id="message" class="error"><p><?php printf(esc_html__('WooCommerce Booking Exporter requires WooCommerce and WooCommerce Booking to be installed!', 'wbe-exporter')); ?></p></div>
            <?php
        }
    }
    $wbe = new WBE();
